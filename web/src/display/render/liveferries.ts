...
interface FerryMsg { id: number; name: string; lat: number; lon: number; speed: number; atDock: boolean; route: string; }
        ...
        for (const m of j.ferries as FerryMsg[]) {
          ...
          if (v) {
            ...
            vessels.set(m.id, {
              ...
              lat: m.lat, lon: m.lon, alat: m.lat, alon: m.lon, tLat: m.lat, tLon: m.lon, lastSeen: now,
            });
          }
        }
      })
