...
const C = {
  ...
  header: { font: "500 12px system-ui", letterSpacing: ".04em", textTransform: "uppercase",
    color: "#8a8f98", padding: "0 16px 8px" } as const,
  row: { display: "flex", alignItems: "center", justifyContent: "space-between",
    gap: 12, padding: "12px 16px", minHeight: 44, borderTop: "0.5px solid #e6e6ea" } as const,
};
...
export function ListRow({ label, children, first }: { label: string; children?: ReactNode; first?: boolean }) {
  return (
    <div style={{ ...C.row, borderTop: first ? "none" : C.row.borderTop }}>
      <span style={{ font: "400 16px system-ui", color: "#1c1c1e" }}>{label}</span>
      <span>{children}</span>
    </div>
  );
}
  ...
  return (
    <button aria-pressed={value} onClick={() => onChange(!value)}
      style={{ width: 51, height: 31, borderRadius: 999, border: "none", padding: 2, cursor: "pointer",
        background: value ? "#34c759" : "#e4e4e9", transition: "background .15s" }}>
...
export function Segmented<T extends string>(
  ...
  return (
    <div style={{ display: "inline-flex", background: "#e9e9ee", borderRadius: 9, padding: 2 }}>
      {options.map((o) => (
        <button key={o.value} onClick={() => onChange(o.value)}
          style={{ font: "500 13px system-ui", padding: "5px 12px", border: "none", borderRadius: 7, cursor: "pointer",
            background: value === o.value ? "#fff" : "transparent",
            color: value === o.value ? "#1c1c1e" : "#3c3c43",
