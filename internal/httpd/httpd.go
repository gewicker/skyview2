...
func New(d Deps) http.Handler {
	...
	// Live bus positions (OBA). Empty when disabled.
	mux.HandleFunc("/api/buses", func(w http.ResponseWriter, r *http.Request) {
		if d.Buses == nil {
			writeJSON(w, map[string]any{"buses": []any{}, "updated": 0})
			return
		}
		writeJSON(w, d.Buses())
	})
